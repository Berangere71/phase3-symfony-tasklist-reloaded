 ...
 */
final class Version20260424141623 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__task AS SELECT id, title, status, is_pinned FROM task');
        $this->addSql('DROP TABLE task');
        $this->addSql('CREATE TABLE task (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(255) NOT NULL, status VARCHAR(255) NOT NULL, is_pinned BOOLEAN NOT NULL, priority VARCHAR(50) DEFAULT NULL, folder VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO task (id, title, status, is_pinned) SELECT id, title, status, is_pinned FROM __temp__task');
        $this->addSql('DROP TABLE __temp__task');
